...
\\ \indent Kolejnym krokiem było testowanie algorytmów. W tym celu napisane zostały testy jednostkowe, które zestawiały przeciwko sobie poszczególne heurystyki. Na podstawie wyników tych starć dokonano analizy porównawczej. Podjęto również próbę analizy starć między algorytmami oraz ludźmi.
\\ \indent W ostatniej częsci pracy podsumowane zostały wyniki, oraz wypisane zostały wnioski - co można było zrobić lepiej, jakie są możliwości rozwoju aplikacji.
}
...
\newpage
\abstract {
The purpose of this study was to implement the strategy game Battleships and to develop decision-making algorithms to analyze the effectiveness of the various algorithms. According to the initial assumptions, the algorithms were to be based on heuristics. The effectiveness analysis was to be based on direct clashes between different algorithms and, if enough data could be collected, on human-algorithm clashes.
...
\\ \indent In the last part of the work the results were summarized, and conclusions were written out - what could have been done better, what are the possibilities for the development of the application.
}
...
\input{tex/2-statki}    % Umożliwia to również łatwą migrację do nowej wersji szablonu:
\input{tex/3-heurystyki}
\input{tex/4-implementacja}
\input{tex/5-przeglad}
\input{tex/6-analiza}
\input{tex/7-wnioski}
